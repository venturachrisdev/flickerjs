function Router(){
    ...
    var routerProto = function(req,res,next){
        ...
        function internext(err){
            if(!err){
                ...
                return routerProto.err(req,res,internext,err,next);
            }
        }
    };
  ...
  routerProto.err = function(req,res,next,err,internext){
    let fn = routerProto.stack[routerProto.stack.length -1].handler;
        if(fn.length == 4){
